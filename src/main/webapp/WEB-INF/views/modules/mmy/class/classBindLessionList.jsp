...
<script type="text/javascript">
	...
	function cancel(lessionId,lessionName) {
		if(!confirm('请确定是否取消下发课文:'+lessionName)){
			return ;
		}
...
<body>
	<ul class="nav nav-tabs">
		...
		<li  class="active"><a href="${ctx}/operator/class/getLessionByClassId?id=${classInfo.id}">班级下发课文表</a></li>
	</ul><sys:message content="${message}" />
	<table id="contentTable"
		...
		<tbody id="tbody">
			<c:forEach items="${lessionList}" var="lession" varStatus="status">
				<tr id="tr${status.index }">
					...
					<td  id="cancelIssue"><input type="button" class="btn" value="移除下发" onclick="cancel('${lession.id}','${lession.name }')"></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
